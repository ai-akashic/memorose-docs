...
Memorose 是一个面向智能体（Agent）的记忆运行时（Memory Runtime）。系统接收原始事件，将其整合为持久化记忆，推导出高阶洞察（Insight），追踪目标执行状态，并通过 Rust 服务端、仪表盘（Dashboard）和基于 Raft 的部署原语对外暴露所有能力。
...
graph TB
  ...
  style Gateway fill:#f3f4f6,stroke:#9ca3af,color:#111827
