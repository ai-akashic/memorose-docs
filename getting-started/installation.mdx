...
Create `config.toml` yourself using the current schema from `crates/memorose-common/src/config.rs`.

Do not assume `config.example.toml` is fully current. It contains older sections such as `[server]`, `[database]`, and `[consolidation]` that do not match the runtime loader exactly.
